# Step 4: Apex Parser — Core Structure

> **Agent Handoff Context**: Steps 1-3 are COMPLETE.
> - Project skeleton exists at `d:\Git\tree-sitter-salesforce\`
> - `common/` has shared utilities (`ci()`, `commaJoined()`, Salesforce types)
> - `soql/` has a working SOQL parser with 30+ tests passing
> - All tooling verified working (`tree-sitter generate`, `tree-sitter test`)

## Context

This step builds the **foundation of the Apex parser** — the core type system, declarations,
and basic expressions. Apex is a Java-like language, so we use `tree-sitter-java` as
structural inspiration while adding Salesforce-specific features.

### Apex vs. Java — Key Differences for the Parser

| Feature | Java | Apex |
|---|---|---|
| Case sensitivity | Case-sensitive | **Case-insensitive** (keywords only) |
| Sharing model | N/A | `with sharing`, `without sharing`, `inherited sharing` |
| DML statements | N/A | `insert`, `update`, `delete`, `upsert`, `undelete`, `merge` |
| Inline queries | N/A | `[SELECT Id FROM Account]` |
| Triggers | N/A | `trigger MyTrigger on Account (before insert) { }` |
| Access modifiers | public, private, protected | public, private, protected, **global** |
| Generics | Full generics | Limited to collections (`List<>`, `Set<>`, `Map<>`) |
| Annotations | `@Override`, etc. | `@IsTest`, `@AuraEnabled`, `@Future`, etc. |
| Properties | No language-level properties | `public String Name { get; set; }` |
| Null coalescing | N/A (added Java 14 discussion) | `??` operator (API v59+) |
| Safe navigation | N/A | `?.` operator (API v50+) |

### Grammar Architecture — Breadth-First Approach

Tree-sitter documentation recommends building grammars **breadth-first**: start with the
top-level structure and work down to details. We follow this order:

```
source_file                        ← Step 4 (this step)
├── class_declaration              ← Step 4
│   ├── modifiers                  ← Step 4
│   ├── type_identifier            ← Step 4
│   ├── class_body                 ← Step 4
│   │   ├── field_declaration      ← Step 4
│   │   ├── method_declaration     ← Step 5
│   │   ├── constructor            ← Step 5
│   │   └── property_declaration   ← Step 5
│   └── ...
├── interface_declaration          ← Step 4
├── enum_declaration               ← Step 4
├── trigger_declaration            ← Step 4
├── expressions                    ← Step 4 (basic), Step 5-6 (advanced)
├── statements                     ← Step 5
└── annotations                    ← Step 6
```
...
- Steps 1-3 complete
- SOQL parser working and tested

## Objectives

After completing this step, you will have:

- [x] `apex/grammar.js` with core Apex grammar (~500-700 lines)
- [x] Declarations: class, interface, enum, trigger
- [x] Type system: primitives, sObjects, generics, collections, void
- [x] Modifiers: access, static, final, override, virtual, abstract, sharing
- [x] Basic expressions: literals, identifiers, member access, `this`, `super`
- [x] Operator precedence table matching Salesforce documentation
- [x] Comments: line (//) and block (/* */)
- [x] Test corpus for all implemented constructs
- [x] `tree-sitter generate` and `tree-sitter test` pass

## Detailed Instructions

### 4.1 Write Test Corpus FIRST

#### 4.1.1 Declaration Tests

Create `d:\Git\tree-sitter-salesforce\apex\test\corpus\declarations.txt`:
...
==================
Global class with multiple interfaces
==================

global class MyService implements Queueable, Database.Batchable {
}
...
(source_file
  (class_declaration
    (modifiers (modifier))
    name: (identifier)
    (interfaces (type_identifier) (type_identifier))
    body: (class_body)))
...
Create `d:\Git\tree-sitter-salesforce\apex\test\corpus\literals.txt`:

```
==================
String literal in field
==================
...
#### 4.1.3 Comments Tests

Create `d:\Git\tree-sitter-salesforce\apex\test\corpus\comments.txt`:
...
### 4.2 Implement the Apex Core Grammar

Replace `d:\Git\tree-sitter-salesforce\apex\grammar.js` with the core grammar.

**Due to the size of the Apex grammar (~700+ lines), the complete implementation is provided
below. Every rule is documented with comments explaining what it does and why.**
...
> **IMPORTANT**: The grammar above is a starting point. You will likely need to:
> 1. Adjust `conflicts` as you add rules (tree-sitter will tell you which rules conflict)
> 2. Adjust test expectations to match the actual tree output
> 3. Run `npx tree-sitter test -u` to auto-update tests, then review changes
>
> Common issues at this stage:
> - **"Unresolved conflict"** during `generate`: Add the conflicting rule pair to `conflicts`
> - **Keywords matching as identifiers**: Check that `word: ($) => $.identifier` is set
> - **Modifier not matching**: Ensure `ci()` is imported from `common/common.js`

### 4.4 Test with Real Apex Code

```powershell
cd d:\Git\tree-sitter-salesforce\apex

# Create a sample .cls file with the structure from apex-recipes
@"
public with sharing class AccountTriggerHandler extends TriggerHandler {
    private List<Account> triggerNew;
    private Map<Id, Account> triggerMapNew;
}
"@ | Out-File -Encoding utf8 test-real.cls

npx tree-sitter parse test-real.cls
...
git add apex/
git commit -m "feat(apex): core grammar with declarations, types, and expressions

- Class, interface, enum, trigger declarations
- Type system: primitives, sObjects, generics (List<>, Map<>, Set<>), arrays
- Modifiers: access, sharing, static, final, virtual, abstract, override
- Basic expressions: literals, identifiers, this, super
- Field declarations with initializers
- Comments (line and block)
- Operator precedence table from Salesforce documentation
- Test corpus with 15+ test cases"
```

## Verification Checklist

- [ ] `apex/grammar.js` contains the core Apex grammar
- [ ] `apex/test/corpus/declarations.txt` has 12+ test cases
- [ ] `apex/test/corpus/literals.txt` has 4+ test cases
- [ ] `apex/test/corpus/comments.txt` has 3+ test cases
- [ ] `cd apex && npx tree-sitter generate` succeeds
- [ ] `cd apex && npx tree-sitter test` — all tests pass
- [ ] `npx tree-sitter parse test-real.cls` parses a real Apex class file
...
```
apex/
├── grammar.js              ✅ Core grammar (~700 lines)
├── package.json             ✅ Minimal
├── src/                     ✅ Generated with core parser
├── test/
│   └── corpus/
│       ├── declarations.txt ✅ Class, interface, enum, trigger tests
│       ├── literals.txt     ✅ String, int, boolean, null tests
│       └── comments.txt     ✅ Line and block comment tests
└── queries/                 📝 Still placeholders
```

**What the parser can parse at this point:**
- ✅ Class declarations with modifiers, extends, implements
...
**What's NOT yet supported (coming in Steps 5-6):**
- ❌ Method declarations
- ❌ Constructors and properties
- ❌ If/else, for, while, switch/when
- ❌ DML statements
- ❌ SOQL/SOSL expressions
- ❌ Annotations
- ❌ Binary/unary/ternary operators
- ❌ New expressions

**Next step:** Step 5 — Apex Statements & Control Flow
